...
test('if.start / if.end gates execution', async () => {
  const steps: WorkflowStep[] = [
    { action: 'if.start', key: 'test', condition: 'eq', value: 'run' } as any,
    { action: 'transform.default_value', key: 'x', value: 'yes' } as any,
    { action: 'if.end' } as any,
  ];
  ...
  expect((result1.ctx as any).x).toBeUndefined();
  ...
  expect((result2.ctx as any).x).toBe('yes');
});

test('create_or_update defaults then increments', async () => {
  const steps: WorkflowStep[] = [
    { action: 'create_or_update', key: 'counter', increment_by: '1', default_value: '0' } as any,
    { action: 'create_or_update', key: 'counter', increment_by: 2, default_value: 0 } as any,
  ];

  const result = await runWorkflowSteps({ initialCtx: {}, steps });
  expect(result.status).toBe('success');
  if (result.status !== 'success') throw new Error('Expected success');
  expect((result.ctx as any).counter).toBe(2);
});

test('while.start / while.end loops until condition fails', async () => {
  const steps: WorkflowStep[] = [
    { action: 'while.start', key: 'counter', condition: 'lt', value: 3 } as any,
    { action: 'create_or_update', key: 'counter', increment_by: 1, default_value: 0 } as any,
    { action: 'while.end' } as any,
  ];

  const result = await runWorkflowSteps({ initialCtx: { counter: 0 }, steps });
  expect(result.status).toBe('success');
  if (result.status !== 'success') throw new Error('Expected success');
  expect((result.ctx as any).counter).toBe(3);
});
